defmodule Taskee.ProjectsTest do
  ...
  describe "projects" do
    ...
    test "create_project/1 with valid data creates a project" do
      valid_attrs = %{description: "some description", due_date: ~U[2023-06-04 12:04:00Z], name: "some name"}

      assert {:ok, %Project{} = project} = Projects.create_project(valid_attrs)
      assert project.description == "some description"
      assert project.due_date == ~U[2023-06-04 12:04:00Z]
    ...
    test "update_project/2 with valid data updates the project" do
      project = project_fixture()
      update_attrs = %{description: "some updated description", due_date: ~U[2023-06-05 12:04:00Z], name: "some updated name"}

      assert {:ok, %Project{} = project} = Projects.update_project(project, update_attrs)
      assert project.description == "some updated description"
      assert project.due_date == ~U[2023-06-05 12:04:00Z]
  ...
  describe "tasks" do
    ...
    @invalid_attrs %{description: nil, due_date: nil, is_completed: nil, name: nil}
    ...
    test "create_task/1 with valid data creates a task" do
      valid_attrs = %{description: "some description", due_date: ~U[2023-06-04 12:07:00Z], is_completed: true, name: "some name"}

      assert {:ok, %Task{} = task} = Projects.create_task(valid_attrs)
      assert task.description == "some description"
      assert task.due_date == ~U[2023-06-04 12:07:00Z]
    ...
    test "update_task/2 with valid data updates the task" do
      task = task_fixture()
      update_attrs = %{description: "some updated description", due_date: ~U[2023-06-05 12:07:00Z], is_completed: false, name: "some updated name"}

      assert {:ok, %Task{} = task} = Projects.update_task(task, update_attrs)
      assert task.description == "some updated description"
      assert task.due_date == ~U[2023-06-05 12:07:00Z]
